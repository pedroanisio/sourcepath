...
Use the correct path for the file's depth: root → `./DISCLAIMER.md`, one level → `../DISCLAIMER.md`, two levels → `../../DISCLAIMER.md`. Place after the title, before the first content section. Never omit it.
...
`codebase-mapper` maps source repositories into RDF and JSON bundle artifacts
that can be inspected through a CLI, FastAPI backend, React UI, and read-only
MCP server. Its purpose is defined in `PURPOSE.md`; agent decisions should keep
mechanically derived facts, generated inferences, and LLM-authored enrichment
clearly separated.
...
├── PURPOSE.md         # Project purpose and non-goals
├── README.md
├── codebase_mapper/   # Python package
├── plugins/           # Extension layers
├── frontend/          # FastAPI, MCP server, and React UI
└── tests/             # Contract and drift verifiers
```
