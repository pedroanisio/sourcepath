...
# Bump on every change to BuildStep/BuildPlan.to_dict() shape or to the
# generation *behavior* (new step kinds, new skip conditions, reordering
# rules). The free-text `provenance.tool` string is for humans; this is for
# programmatic consumers who need to know whether two runs are comparable.
...
class BuildPlan:
    ...
    # Files the decomposition carries that no step creates or modifies, each
    # with a reason (e.g. "regenerated by its build tool, not hand-written").
    # Reconciliation happens once, at generation time, so omission is always a
    # recorded decision, never an accident (Part IV: nothing is silently
    # dropped).
    unassigned_files: list[dict[str, str]] = field(default_factory=list)
