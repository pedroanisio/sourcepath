...
| **COBOL**       | **Promoted to Tier-1** (2026-07-09) — no PyPI tree-sitter wheel, so a self-contained **column-aware** regex reader (`extraction_method: "regex"`) handling **both fixed- and free-format** source (per-file `>> SOURCE FORMAT` / heuristic detection). Structural map: program → `top_level_classes` (chunk `class`); PROCEDURE DIVISION section/paragraph → `top_level_functions` (chunk `method`, parent = program). Xrefs: `PERFORM` → intra-program `calls` (incl. `THRU`), `CALL 'literal'` → inter-program `calls`, `CALL identifier` → unresolved `dynamic_dispatch`; `COPY` copybooks → `ImportEdge` (basename resolution, no host index). Conditional items legitimately skipped: no tree-sitter grammar (C2.2.1-4), no dependency manifest/lockfile (C2.6/C2.7 — COBOL has none), no filename test convention (C2.1.2). Documented v1 limit: DATA DIVISION data items are not a symbol surface. Verified by [`tests/verify_cobol.py`](../tests/verify_cobol.py) (35/35). | ✔ done             |
